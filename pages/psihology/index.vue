...
export default {
  head: {
    title: `Психология | ${process.env.appName}`
  },
  layout: 'category',
  components: {
    Post,
  ...
  async asyncData({ store }) {
    ...
    const posts = allPosts.filter(post => post.category === 'psihology')
    return { posts }
  },
  data: () => ({
    paralax: {
      picture: '/category/psihology.jpg',
      category: 'Психология',
      title: 'психология людей, отношения, полезные привычки'
    }
  })
}
</script>